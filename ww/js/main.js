...
var rootURL = "http://staging.eimpressive.com/fish/api/wine";
...
/*	$('#wineList li').remove();
	$.each(list, function(index, wine) {
		$('#wineList').append('<li><a href="#" data-identity="' + wine.user_id + '">'+wine.firstnameb+'</a></li>');
	});
}

function renderDetails(wine) {
	$('#wineId').val(usera.user_id);
	$('#firstnameb').val(wine.firstnameb);
	$('#lastnameb').val(wine.grapes);
	$('#usernameb').val(wine.country);
	$('#passwordb').val(wine.region);
	$('#year').val(wine.year);
	$('#pic').attr('src', 'pics/' + wine.picture);
	$('#emailidb').val(wine.description);
}
...
function formToJSON() {
	return JSON.stringify({
		...
		"usernameb": $('#usernameb').val(),
		"passwordb": $('#passwordb').val(),
		"year": $('#year').val(),
		"picture": currentWine.picture,
		"emailidb": $('#emailidb').val()
		});
}
*/